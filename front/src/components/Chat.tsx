...
export const Chat = () => {
  const { fetchData: saveMessageApi } = useLazyApi<SaveMessageResponse>(
    BackendEndpoints.SaveMessage
  );
  const { data, fetchData: fetchChatHistoryApi } =
    useLazyApi<ChatHistoryResponse>(BackendEndpoints.ChatHistory);
  const { data: askQueryData, fetchData: askQueryApi } =
    useLazyApi<AskQueryResponse>(BackendEndpoints.AskQuery);
    const { userId } = useAuth();
  const { pageId, messageId } = useParams<{
    pageId: string;
    messageId: string;
  }>();
  ...
  const [currentPage, setCurrentPage] = useState<string>(
    ...
    const loadChatHistory = useCallback(async (pageUuid: string) => {
    ...
    }, [userId, fetchChatHistoryApi, data]);
    ...
    const saveMessage = useCallback(async (messageDetails: {
        userId: string,
        messageId: string,
        content: string,
        type: "sent" | "received",
    ...
    >
      <Group style={{ position: "relative", zIndex: 1 }}>
          ...
          <Button p={3} variant="subtle" onClick={handleNewChat}>
                        <IconPlus size={20} />
                        New Chat
                    </Button>
                </Group>
                ...
                style={{ cursor: "pointer" }}
                  ...
                  maw="70%"
                    ...
                    borderBottomRightRadius:
                      message.type === "sent" ? 0 : rem(16),
                    borderTopLeftRadius:
                      message.type === "received" ? 0 : rem(16),
                                    maxWidth: "70%",
                                    backgroundColor:
                                        message.type === "sent"
